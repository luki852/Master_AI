...
if __name__ == "__main__":
    ...
    # Enforce maximum submission file size
    max_size_bytes = 50 * 1024 * 1024  # 50 MB
    submission_size = os.path.getsize(model_file)
    if submission_size > max_size_bytes:
        raise ValueError(
            f"Submission file exceeds maximum allowed size of 50 MB "
            f"(got {submission_size / (1024 * 1024):.2f} MB)."
        )
